  ...
  $(".save_coupon_btn").click(function(){
    ...
    var sectionId = $("#vendor_vendor_select option:selected").attr('sectionId');
      ...
      if(code == ''){
        ...
        database.collection('coupons').doc(id).update({'code':code,'description':description,'discount':discount,'expiresAt':expiresAt,'isEnabled':isEnabled,'id':id,'discountType':discountType,'image':photo_coupon,'vendorID':vendorID,'section_id':sectionId,'isPublic':isPublic}).then(function(result) {
